...
namespace MainClient.Scheduler
{
    ...
    public enum TrafficProxyIpKind
    {
        Fetched,
        Consumed
    }

    public record TrafficTaskStateEvent(
        int TaskId,
        TrafficTaskStateKind State,
        int Count,
        string? Data = null);

    public record TrafficTaskProxyIpStateEvent(
        int TaskId,
        TrafficProxyIpKind Kind,
        string? Ip = null,
        int Count = 1
    );

    public readonly record struct TrafficTaskStateSnapshot(long Start, long Dsp, long Click)
    {
        public bool IsEmpty => Start == 0 && Dsp == 0 && Click == 0;
    }

    public readonly record struct TrafficProxyIpStateSnapshot(long Fetched, long Consumed, string[] ConsumedIps)
    {
    ...
    public sealed class TrafficLocalHourTaskState
    {
        ...
        public TrafficLocalHourTaskState(string hourKey)
        {
            HourKey = hourKey;
    ...
    public sealed class TaskMetricsService : IAsyncDisposable, IDisposable
    {
        #region Fields
        ...
        /// 任务队列
        /// </summary>
        private readonly Channel<TrafficTaskStateEvent> _taskStateQueue;
        /// <summary>
        /// Ip队列
        /// </summary>
        private readonly Channel<TrafficTaskProxyIpStateEvent> _proxyIpStateQueue;
        ...
        public TaskMetricsService(
            ...
            ILogger<TaskMetricsService> logger,
            ...
            int retryCount = 3)
        {
            ...
            _taskStateQueue = Channel.CreateUnbounded<TrafficTaskStateEvent>(
            ...
            _proxyIpStateQueue = Channel.CreateUnbounded<TrafficTaskProxyIpStateEvent>(
                new UnboundedChannelOptions
                {
                    SingleReader = true,
                    SingleWriter = false,
        ...
        private void StartBackgroundWorkers()
        {
            ...
            if (state == 4)
                throw new ObjectDisposedException(nameof(TaskMetricsService));
        ...
        /// <returns></returns>
        public TrafficTaskStateEntity? GetTaskStateById(int taskId) => _taskStates.TryGetValue(taskId, out var stats) ? stats : null;
        ...
        {
            var buffer = new List<TrafficTaskStateEvent>(256);
            ...
            try
            {
                while (await _taskStateQueue.Reader.WaitToReadAsync(token).ConfigureAwait(false))
                {
                    ...
                    {
                        var state = _taskStates.GetOrAdd(item.TaskId, _ => new TrafficTaskStateEntity());

                        if (item.State == TrafficTaskStateKind.X5Sec)
                        {
        ...
        private async Task ProcessProxyIpQueueAsync(CancellationToken token)
        {
            try
            {
                while (await _proxyIpStateQueue.Reader.WaitToReadAsync(token).ConfigureAwait(false))
                {
                    ...
                    {
                        var stat = _proxyIpStates.GetOrAdd(ev.TaskId, _ => new TrafficProxyIpStateEntity());

                        if (ev.Kind == TrafficProxyIpKind.Fetched)
                        {
        ...
        public async Task FlushOnceAsync(CancellationToken token = default)
        {
            ...
            {
                int taskId = pair.Key;
                TrafficTaskStateEntity state = pair.Value;
            ...
            {
                int taskId = pair.Key;
                TrafficProxyIpStateEntity stat = pair.Value;
        ...
        private async Task FlushTaskStateAsync(
            int taskId,
            TrafficTaskStateEntity stats,
            TrafficTaskStateSnapshot snapshot,
            ...
            CancellationToken token)
        {
            ...
            {
                await RetryAsync(
                    () => _adxHelper.UpdateTaskStateAsync(taskId, metrics, token),
                    _retryCount,
        ...
        private async Task FlushProxyIpStateAsync(
            int taskId,
            TrafficProxyIpStateEntity stat,
            TrafficProxyIpStateSnapshot snapshot,
            Dictionary<string, long> metrics,
            CancellationToken token)
        {
            await _flushSemaphore.WaitAsync(token).ConfigureAwait(false);
        ...
        private async Task FlushHostStateAsync(
            TrafficTaskStateSnapshot delta,
            Dictionary<string, long> metrics,
            CancellationToken token)
        {
            await _flushSemaphore.WaitAsync(token).ConfigureAwait(false);
            ...
            {
                await RetryAsync(
                    () => _adxHelper.UpdateHostStateAsync(metrics, token),
                    _retryCount,
                    token).ConfigureAwait(false);
        ...
        private TrafficLocalHourTaskState _localStats = new TrafficLocalHourTaskState(GetHourKey());
        ...
        public void AddLocalMetric(int taskId, string name, long value = 1)
        {
            ...
            while (true)
            {
                ...
                var newStats = new TrafficLocalHourTaskState(hour);
